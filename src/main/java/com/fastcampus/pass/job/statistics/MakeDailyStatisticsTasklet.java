...
public class MakeDailyStatisticsTasklet implements Tasklet {
    ...
    // 저장된 statistics records 를 가져와서 batch.repeat status 를 FINISHED 로 변경
    // CSV 로 추출 하고, 
    @Override
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
        ...
        final List<AggregatedStatistics> statisticsList = statisticsRepository
                .findByStatisticsAtBetweenAndGroupBy(from, to);
        ...
        CustomCSVWriter.write(
                "daily_statistics_" + LocalDateTimeUtils.format(from, LocalDateTimeUtils.YYYY_MM_DD) + ".csv",
                data
        );
