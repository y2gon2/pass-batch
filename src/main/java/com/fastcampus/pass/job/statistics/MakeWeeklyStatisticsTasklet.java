...
public class MakeWeeklyStatisticsTasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
        ...
        final List<AggregatedStatistics> statisticsList =
                statisticsRepository.findByStatisticsAtBetweenAndGroupBy(from, to);

        // key : 1년을 주로 환산한 정수값.
        Map<Integer, AggregatedStatistics> weeklyStatisticsEntityMap = new LinkedMap<>();
        ...
        CustomCSVWriter.write(
                "weekly_statistics_" + LocalDateTimeUtils.format(from, LocalDateTimeUtils.YYYY_MM_DD) + ".csv",
                data
        );
